...
def show_pcl(pcl):
    ...
    vis.create_window(window_name='Visualize lidar point-cloud')
    
    # step 2 : create instance of open3d point-cloud class
    p_cloud = o3d.geometry.PointCloud()

    # step 3 : set points in pcd instance by converting the point-cloud into 3d vectors (using open3d function Vector3dVector)
    p_cloud.points = o3d.utility.Vector3dVector(pcl[:,:3])
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    min_y = np.amin(lidar_pcl_cpy[:,1])     # find min y value
    #lidar_pcl_cpy[:, 1] = lidar_pcl_cpy[:, 1] - min_y    #offset all y values by min_y to ensure only positive y values remain.
    ...
    intensity = lidar_pcl_cpy[idx_intensity]

    ## step 3 : extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    ##          also, store the number of points per x,y-cell in a variable named "counts" for use in the next task
    _, idx_unique, counts = np.unique(intensity[:, 0:2], axis=0, return_index=True, return_counts=True)
    lidar_pcl_top = intensity[idx_unique]
